...
async def predict(data: dataInt):
    try:
        ...
        while current_date <= end_date:
            ...
            predictions.append(prediction.iloc[0])  # Prendre le premier élément de la prédiction
...
async def get_combined_predictions(start_date: Optional[str] = Query(None, description="Date de début au format YYYY-MM-DD"),
    ...
    try:
        ...
        # Afficher les premières lignes des deux DataFrames pour le débogage
        print("Prédictions :")
        ...
        return {"error": f"Erreur lors de la récupération des prédictions combinées avec les données réelles : {str(e)}"}
